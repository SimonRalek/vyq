...
pub const Emitter = struct {
    ...
    pub fn init(allocator: Allocator, vm: *VM, compiler: *Self) Self {
        return .{ .allocator = allocator, .vm = vm, .reporter = vm.reporter, .locals = localArray.init(allocator), .scope_depth = 0, .current = compiler };
    }
